...
bool slove_sat_problem(int roundnum, int ActiveNum) {
    ...
    if (flag == true) {
        cout << "Round:" << to_string(roundnum) << "; " + ProblemType + ": " << ActiveNum/5 << "; Sat; TotalCost: " << (double)((time_end - time_start)/CLOCKS_PER_SEC) << "s" << endl;
    }
    else
        cout << "Round:" << to_string(roundnum) << "; " + ProblemType + ": " << ActiveNum/5 << "; Unsat; TotalCost: " << (double)((time_end - time_start)/CLOCKS_PER_SEC) << "s" << endl;
